...
class CreateAppPort extends React.Component {
    ...
    handleChange = (key, val, item) => {
        ...
        this.setState({
            ...
            if (key === 'portKey') {
                const { getGatewaySelectObj } = this.state
                this.setState({
                    resourceObjectId: '',
                    resourceObjectList: getGatewaySelectObj[value]
                })
            }
        })
    }
    ...
    render() {
        ...
        const { getGatewaySelectObj: selectList } = this.state
